...
## 게임 규칙 (시간 맞추기)
- 매 라운드마다 10~20초 사이의 정수(예: 14초)로 목표 시간이 무작위로 정해집니다. 라운드가 시작되기 전 슬롯머신처럼 숫자가 돌아가다 목표 시간에 멈추는 연출과 함께 미리 공개되고("n초 후 시작" 카운트다운), 실제로 시간이 흐르기 시작한 뒤에도 3초간 더 보이다가 가려집니다.
- 참가자는 그 목표 시간을 기억해뒀다가, 그만큼 시간이 지났다고 생각되는 순간 "지금 멈추기!" 버튼을 눌러 자신의 시간을 기록합니다(실제 경과 시간은 멈추기 전까지 보이지 않습니다).
